...
//            if (c0==0)
    ...
    private abstract class SpectrumData implements MouseListener, MouseMotionListener, Measurements, KeyListener {
        ...
        void updateProfile() {
            ...
            if (pwin == null || imgp == null) {
                imgp = new ImagePlus(plot.getTitle(),plot.getProcessor());
                imgp.show();
                pwin = imgp.getWindow();
            ...
            plot.setBackgroundColor(colBackFill);
            ...
            //ff.fill8(plotFrame.x+(plotFrame.width)/2, plotFrame.y+1);
            ...
            imgp.setProcessor(ipplot);
            marginWidth = pwin.getWidth()- plot.getDrawingFrame().width;
            marginHeight = pwin.getHeight()- plot.getDrawingFrame().height;
        }

        void drawWindow(int xI, int xF, Color c, Plot plot) {
            ImageProcessor ipplot = plot.getProcessor();
            final Rectangle plotFrame = plot.getDrawingFrame();
            ipplot.setColor(c);
            ...
            if ((xIdraw >= 0) && (xIdraw <= size)) {
                ipplot.drawRect((int) (plotFrame.x + plotFrame.width * xIdraw), plot.TOP_MARGIN, 1, (plotHeight));
            }
            if ((xFdraw >= 0) && (xFdraw <= size)) {
                ipplot.drawRect((int) (plotFrame.x + plotFrame.width * xFdraw), plot.TOP_MARGIN, 1, (plotHeight));
            }
        }
        ...
        void updateProgress(double progress) {
            if (progress == 1)
                imgp.setTitle("CSI: Cornell Spectrum Imager - "+img.getTitle());
            else
                imgp.setTitle("(Working: %" + String.format("%.0f", progress * 100) + ") CSI: Cornell Spectrum Imager - "+img.getTitle());
        }
    }
    ...
    private class SpectrumData2D extends SpectrumData {
        ...
        ImagePlus fitToBosman(int fitStart, int fitEnd, int intStart, int intEnd) {
            ...
            JamaDenseDoubleMatrix2D yMatUJMP = new JamaDenseDoubleMatrix2D( yMat );
...
//                IJ.run(imgfilter, "Median...", "radius="+filtersize+" stack");
            ...
            imgp.setTitle("(Working: %50) [Doing Singular Value Composition: may take a few minutes.]  CSI: Cornell Spectrum Imager - "+img.getTitle());
        ...
        void weightedPCA(int fitStart, int fitEnd, int pcaStart, int pcaEnd) {
            ...
            yMatUJMP = new JamaDenseDoubleMatrix2D( yMat);
            ...
            if (meanCentering)
                yMatUJMP.center(Calculation.ORIG, Matrix.ROW, true);
            Matrix[] USV = yMatUJMP.svd();
            imgp.setTitle("(Working: %50) CSI: Cornell Spectrum Imager - "+img.getTitle());
